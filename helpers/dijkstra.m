function [path,dist,steps] = dijkstra(start,finish,vertices,adjacencies,path)
...
%    to every point in the mesh that has been traversed
%    
% Example: 
%    obj = read_object('../test_files/duck.obj');
%    adjacencies = compute_adjacencylist(obj);
%    [path,dist,steps] = dijkstra(1,10,obj.v,adjacencies)
%
% Copyright (c) 2018 Nikolas Lamb
%

% If you'd like to watch the function
vis = false;
...
while ~isempty(valid_pts) 
    ...
    for child = children
        ...
        dist_child = dist_traversed_list(parent) + dist_edge;
